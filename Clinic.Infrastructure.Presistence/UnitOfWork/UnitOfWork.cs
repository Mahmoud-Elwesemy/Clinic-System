...
public class UnitOfWork:IUnitOfWork
{
        ...
        where Tkey : IEquatable<Tkey>
    {
        ...
        var key = $"{typeof(T).FullName}_{typeof(GenericRepository<T,Tkey>).Name}";
        return (IGenericRepository<T,Tkey>) _repositories.GetOrAdd(key,_ =>
            new GenericRepository<T,Tkey>(_context));
    }
    //------------------------------------------------------------------------------------------
    // This Method Is Used To Get A Appointment Repository 
    public IAppointmentRepository GetAppointmentRepository()
    {
        var key = $"{typeof(Appointment).FullName}_{typeof(AppointmentRepository).Name}";
        return (IAppointmentRepository) _repositories.GetOrAdd(key,_ =>
